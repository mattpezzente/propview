...
const searchIcon = require('../images/propview-search-icon-white.png');
...
class SearchBar extends Component {
  ...
  render() {
    return (
      <div className="search-bar-container">
        ...
        <button onClick={this.checkAddress} className="search-address-button"><img alt="search button for entered address" src={searchIcon}/></button>
      </div>
    );
  }
  ...
  checkAddress() {
    ...
    const address = this.validateAddress(document.querySelector('.search-address').value);
    ...
    if (this.localProps.loading) {
      ...
      const sendData = () => {
        ...
        propData = Object.assign(
          {},
          apiObjects.onProperty,
          apiObjects.onSchools,
          apiObjects.onAVM,
          apiObjects.onSale,
          apiObjects.zillSearch,
          apiObjects.zillProperty
        );
      ...
      axios(confOnSale)
        .then((dataOnSalesHistory) => {
          ...
          axios(Object.assign(
            ...
            {
              params: {
                latitude: propLongLat[0],
                longitude: propLongLat[1],
                radius: 15
              }
            }
          ))
      ...
      axios(confZillowDeepSearch)
        .then((dataZillSearch) => {
          let localDataZillSearch = dataZillSearch;
          finishedAPIs.zillSearch = true;
          localDataZillSearch = convert.xml2js(dataZillSearch.data, { compact: true, spaces: 2 })['SearchResults:searchresults'].response.results.result;
          apiObjects.zillSearch = localDataZillSearch;
          localDataZillSearch = localDataZillSearch.zpid._text;
  ...
  validateAddress(address) {
    const addressLines = [];
    let methodAddress = address;
    let line1 = '';
    let line2 = '';

    methodAddress = parser.parseLocation(address);

    if (methodAddress !== null && methodAddress.number && methodAddress.street && methodAddress.type && methodAddress.city && methodAddress.state) {
      line1 += methodAddress.number + ' ';
      if (methodAddress.prefix) {
        line1 += methodAddress.prefix + ' ';
      }
      line1 += methodAddress.street + ' ';
      line1 += methodAddress.type + ' ';


      line2 += methodAddress.city + ' ';
      line2 += methodAddress.state + ' ';
      if (methodAddress.zip) {
        line2 += methodAddress.zip;
      }
